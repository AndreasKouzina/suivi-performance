...
// Permet de simuler la rentabilité d'un événement/prestation exceptionnelle
// avant de s'engager (mode "simulation"), puis de basculer en suivi réel une
// fois l'événement confirmé/réalisé (mode "reel") — avec comparaison
// prévisionnel vs réel côte à côte. Les projets sont stockés dans
// data.projets (colonne app_data.projets côté Supabase), lus/écrits via le
// flux standard sécurisé (safeWriteAppData / updData) — pas de sauvegarde
// séparée qui contournerait l'anti-écrasement.
...
function ProjetDetail({projet, cats, onBack, onUpdate, moisData, onUpdateMois}){
  ...
  // CORRECTIF ANTI-ÉCRASEMENT : ajoute une charge réelle du projet au mois
  // courant en passant par un mutateur fonctionnel (onUpdateMois = upd),
  // qui recharge Supabase juste avant d'écrire — jamais un objet moisData
  // capturé au rendu, potentiellement périmé.
  ...
  const basculerReel=()=>{
    ...
    onUpdateMois(freshMois=>{
      ...
      const encaissements=[...(ev.encaissements||[]),newEnc];
      return {...freshMois,pdv:{...freshMois.pdv,evenementiel:{ca:(n(ev.ca)+caReel),encaissements}}};
    });
  ...
  return <div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8,marginBottom:20}}>
      ...
      <button onClick={exportTxt} style={{...base,background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 14px",cursor:"pointer",fontSize:12,fontWeight:500}}>
        📄 Exporter
      </button>
    </div>
...
function PanneauProjets({data, moisData, onUpdateMois, onUpdateData}){
  const [vue,setVue]=useState("liste"); // liste | nouveau | detail
  const [projetSelecId,setProjetSelecId]=useState(null);
  const cats = data.laboCats;
  const projets = data.projets||[];
  const projetSelec = projets.find(p=>p.id===projetSelecId)||null;

  // CORRECTIF ANTI-ÉCRASEMENT : sauvegarde via onUpdateData (= updData dans
  // AppPatron), qui recharge Supabase juste avant d'écrire — jamais un
  // tableau `projets` local potentiellement périmé.
  const sauvegarder=(p)=>{
    onUpdateData(fresh=>{
      const freshProjets = fresh.projets||[];
      const idx=freshProjets.findIndex(x=>x.id===p.id);
      const newList = idx>=0 ? freshProjets.map(x=>x.id===p.id?p:x) : [...freshProjets,p];
      return {...fresh, projets:newList};
    });
    setProjetSelecId(p.id);
    setVue("detail");
  };

  const supprimer=(id)=>{
    onUpdateData(fresh=>({...fresh, projets:(fresh.projets||[]).filter(p=>p.id!==id)}));
    setVue("liste");
  };
  ...
  return <div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {[...projets].sort((a,b)=>b.createdAt?.localeCompare(a.createdAt||"")||0).map(p=>{
        ...
        return <Card key={p.id} pad={16} style={{cursor:"pointer"}} onClick={()=>{setProjetSelecId(p.id);setVue("detail");}}>
